function [average_delivery_delay,delivery_rate,average_hop_count,average_buffer_occupancy,N_forwarding]= Epidemic_revised(N,destination,filename,Number_of_messages,interarrival_time,buffer_limit,exchange_limit,message_creation_node,TTL)
...
while (i<=length(events{5}))
    ...
    for j=1:end_of_buffer1
        if (acks(node2,nodes_buffer(node1,j))==1)
        ...
        elseif(events{1}(i)-message_creation_time(nodes_buffer(node1,j))>TTL)
            jj=find(buffer_status_node1==nodes_buffer(node1,j));
            buffer_status_node1(jj)=0;
            buffer_status_node1([jj:length(buffer_status_node1)])=buffer_status_node1([jj+1:length(buffer_status_node1) jj]);
        end
    ...
    for j=1:end_of_buffer2
        if (acks(node1,nodes_buffer(node2,j))==1)
        ...
        elseif(events{1}(i)-message_creation_time(nodes_buffer(node2,j))>TTL)
            jj=find(buffer_status_node2==nodes_buffer(node2,j));
            buffer_status_node2(jj)=0;
            buffer_status_node2([jj:length(buffer_status_node2)])=buffer_status_node2([jj+1:length(buffer_status_node2) jj]); 
        end
