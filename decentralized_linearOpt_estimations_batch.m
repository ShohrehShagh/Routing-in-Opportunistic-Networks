function [message_delivery_time]= decentralized_linearOpt_estimations_batch(N,destination,filename,Number_of_messages,interarrival_time,buffer_limit,exchange_limit,message_creation_node,TTL,message_creation_time)
...
number_of_meetings=zeros(N,N);
last_time_met=zeros(N,N);
temp_meeting_rate=zeros(N,N);
meeting_rates=zeros(N,N,length(events{1}));
Limit=3
...
while (i<= length(events{5}))
    ...
    number_of_meetings(node2,node1)=number_of_meetings(node2,node1)+1;
    if (number_of_meetings(node1,node2)==1)
        x=events{1}(i)-last_time_met(node1,node2);
        temp_meeting_rate(node1,node2)=1/x;
        
        temp_meeting_rate(node2,node1)=temp_meeting_rate(node1,node2);
        last_time_met(node1,node2)=events{1}(i);
        last_time_met(node2,node1)=last_time_met(node1,node2);
    elseif (number_of_meetings(node1,node2)<Limit)
        n=number_of_meetings(node1,node2);
        x=events{1}(i)-last_time_met(node1,node2);
        temp_meeting_rate(node1,node2)=(n*temp_meeting_rate(node1,node2))/(n-1+temp_meeting_rate(node1,node2)*x);
        last_time_met(node1,node2)=events{1}(i);
        
        temp_meeting_rate(node2,node1)=temp_meeting_rate(node1,node2);
        last_time_met(node1,node2)=last_time_met(node1,node2);
        if (number_of_meetings(node1,node2)==Limit-1)
            meeting_rates(node1,node2)=temp_meeting_rate(node2,node1);
            meeting_rates(node2,node1)=temp_meeting_rate(node2,node1);
        end
    else
        n=number_of_meetings(node1,node2);
        x=events{1}(i)-last_time_met(node1,node2);
        %%estimating lambdas
        meeting_rates(node1,node2)=(n*meeting_rates(node1,node2))/(n-1+meeting_rates(node1,node2)*x);
        last_time_met(node1,node2)=events{1}(i);
        
        meeting_rates(node2,node1)=meeting_rates(node1,node2);
        last_time_met(node1,node2)=last_time_met(node1,node2);
...
for j=1:Number_of_messages-1
    if(message_state(j,1)=='D' && message_state(j,2)=='E')
        d=d+1;
        ...
        average_hop_count=average_hop_count+hop_count(j);
    end
end
