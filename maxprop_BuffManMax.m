function [average_delivery_delay,delivery_rate,average_hop_count,average_buffer_occupancy,N_forwarding]= maxprop_BuffManMax(N,destination,filename,Number_of_messages,interarrival_time,buffer_limit,exchange_limit,message_creation_node)
...
while (i<=length(events{5}))
    ...
    if (Forward(node1,node2)==1 && exchange1to2>0)
        while(exchange1to2>buffer_limit-end_of_buffer2)
            [a,b]=max(hop_count(node2,:))
        end
